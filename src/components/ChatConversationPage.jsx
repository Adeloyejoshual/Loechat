...
export default function ChatConversationPage() {
  ...
  const sendMessage = async (textMsg = "", files = []) => {
    ...
    if (files.length) {
      for (const f of files) {
        await sendMediaMessage(f, messagesCol);
      }
    }

    // -------------------- Text message --------------------
    if (textMsg.trim()) {
      await sendTextMessage(textMsg, messagesCol);
    }
  ...
  const sendTextMessage = async (textMsg, messagesCol) => {
  ...
  const sendMediaMessage = async (file, messagesCol) => {
    ...
    try {
      ...
      const payload = { ...tempMessage, mediaUrl, createdAt: serverTimestamp(), status: "sent" };
      const docRef = await addDoc(messagesCol, payload);
      setMessages((prev) =>
        prev.map((m) => (m.id === tempId ? { ...payload, id: docRef.id, createdAt: new Date() } : m))
      );
  ...
  return (
    ...
    >
      ...
      <ChatInput
        ...
        uploadFiles={(file) => sendMediaMessage(file, collection(db, "chats", chatId, "messages"))}
